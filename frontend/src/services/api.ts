import { saveOfflineEvent, getOfflineEvents, clearOfflineEvents, saveToCache, getFromCache, removeFromCache, isOnline } from './storage';
...
export const api = {
  ...
  createUser: async (user: Partial<User>) => {
    ...
    const res = await fetchJSON<{ id: number }>('/users', { method: 'POST', body: JSON.stringify(user) });
    removeFromCache('users');
    return res;
  },
  ...
  explainInsight: (domain: string, status: string, evidence: string) =>
    fetchJSON<{ explanation: string }>('/explain/insight', {
      method: 'POST',
